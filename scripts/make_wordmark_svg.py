...
# ---- geometry / grid ------------------------------------------------------
# 50 columns fit the reference "AVI" (3 letters) at ~30 cols/letter, wide
# enough for the extrusion to keep counters open. "RUSHI" is 5 letters, so it
# needs proportionally more columns (90) or the letters collapse into blobs --
# verified with --preview before landing on this value. That makes the panel
# too wide to sit beside the portrait at the reference's proportions, so this
# profile stacks the portrait above the wordmark instead of side-by-side.
COLS = int(os.environ.get("WORDMARK_COLS", 90))
...
# Futura isn't on Windows; Arial Bold is the closest even-stroke-weight
# substitute available by default on this machine (single-face .ttf, so
# FONT_INDEX is 0 here instead of Futura.ttc's face index 2).
...
TEXT = os.environ.get("WORDMARK_TEXT", "RUSHI")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">rushi@github: ~$ ./wordmark.sh --3d</text>')
